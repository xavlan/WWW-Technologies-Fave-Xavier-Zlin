...
export function ProductCard({ component }: ProductCardProps) {
  ...
  const handleCompare = (e: React.MouseEvent) => {
    ...
    if (!component.category) {
      ...
      toast.error('Component category is missing');
  ...
  return (
    ...
    <Card className="group h-full overflow-hidden border-border/60 transition-all hover:border-primary/30 hover:shadow-lg">
      ...
      <CardHeader className="space-y-2 pb-2">
        ...
        <div className="flex items-start justify-between gap-2">
          ...
          <CardTitle className="line-clamp-2 text-base leading-snug">{component.name}</CardTitle>
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          <Badge variant="outline" className="text-xs">{component.brand}</Badge>

          {component.category && (

            <Badge variant="secondary" className="text-xs">{component.category.name}</Badge>
      ...
      <CardContent className="pb-3">
        ...
        <p className="text-2xl font-bold text-primary">${Number(component.price).toFixed(2)}</p>
